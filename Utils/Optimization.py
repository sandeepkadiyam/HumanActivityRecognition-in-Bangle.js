...
class OptimizeModel:
    ...
    Arguments:
        ...
        assertion: bool, should the exported model be tested for compatibility.
    """
    
    def __init__(self, model, dataset, quantize=True, assertion = False):

        self.model = model
        self.dataset = dataset
        self.quantize = quantize
        self.assertion = assertion
        self.model_bytes = self.convert()
        if (self.assertion):
            self.assert_export()
    ...
    def convert(self):
        ...
        if self.quantize:
            ...
            converter.representative_dataset = lambda: ([obs] for obs in (self.dataset).validation.x[np.random.permutation((self.dataset).validation.x.shape[0]), np.newaxis, ...])
    ...
    def assert_export(self):
        ...
        _validate_flatbuffer_for_tflite_micro(self.model_bytes)
    ...
    def areasize(self):
        total_areasize = 0
        model = tflite_schema_model.Model.GetRootAsModel(bytearray(self.model_bytes), 0)
    ...
    def size_report(self):
        ...
        return (
            f'{"Quantized" if self.quantize else "Not-quantized"} model\n'
    ...
    def save(self, filepath):
        ...
        with open(filepath, "wb") as fp:
            fp.write(self.model_bytes)

    def predict(self, input_tensor):

        interpreter = tf.lite.Interpreter(model_content=self.model_bytes)
    ...
    def base64(self):
        return base64.b64encode(self.model_bytes).decode('utf-8')
